...
THINGSPEAK_INTERVAL = 15  # ThingSpeak free tier allows 1 update per 15 seconds
...
# ── ThingSpeak upload queue ───────────────────────────────────────
upload_queue = queue.Queue()
last_upload_time = 0
thingspeak_count = 0
thingspeak_status = "Not uploaded yet"
...
# ── ThingSpeak upload thread ──────────────────────────────────────
def thingspeak_worker():
    global thingspeak_count, thingspeak_status, last_upload_time
    while True:
        try:
            ...
            # Respect ThingSpeak 15 second rate limit
            now = time.time()
            elapsed = now - last_upload_time
            if elapsed < THINGSPEAK_INTERVAL:
                time.sleep(THINGSPEAK_INTERVAL - elapsed)
            ...
            if response.status_code == 200 and response.text != '0':
                ...
                thingspeak_status = f"✅ Upload #{thingspeak_count} OK"
                print(f"[ThingSpeak] Upload #{thingspeak_count} successful — entry {response.text}")
            else:
                thingspeak_status = f"⚠️ Upload failed ({response.status_code})"
                print(f"[ThingSpeak] Upload failed: {response.status_code} — {response.text}")

        except queue.Empty:
            continue
        except Exception as e:
            thingspeak_status = f"❌ Error: {str(e)[:30]}"
            print(f"[ThingSpeak] Error: {e}")

# ── Set up live plot ──────────────────────────────────────────────
fig, axes = plt.subplots(2, 2, figsize=(14, 8))
fig.suptitle('IAQ Live Monitor — ESP32 + ThingSpeak', fontsize=14, fontweight='bold')
...
# Start ThingSpeak upload thread
ts_thread = Thread(target=thingspeak_worker, daemon=True)
ts_thread.start()
...
def update(frame):
        ...
        try:
            ...
            if '=====================================' in line and buffer:
                ...
                if data:
                    ...
                    # Queue for ThingSpeak upload
                    if upload_queue.qsize() < 5:  # don't let queue build up
    ...
    # ThingSpeak status in title
    fig.suptitle(f'IAQ Live Monitor — ESP32 | ThingSpeak: {thingspeak_status} | Readings: {reading_count}',
                 fontsize=11, fontweight='bold')

    plt.tight_layout(pad=3.0)

# ── Run ───────────────────────────────────────────────────────────
print("\n=== IAQ Logger + ThingSpeak Started ===")
print(f"Logging to:       {CSV_FILE}")
print(f"ThingSpeak key:   {THINGSPEAK_API_KEY}")
print(f"ThingSpeak URL:   https://thingspeak.com/channels/YOUR_CHANNEL_ID")
print(f"Upload interval:  every {THINGSPEAK_INTERVAL} seconds (free tier limit)")
...
try:
    ...
    print(f"ThingSpeak uploads:    {thingspeak_count}")
    print(f"Data saved to:         {CSV_FILE}")
    print(f"View online at:        https://thingspeak.com/channels/YOUR_CHANNEL_ID")